"""PostgreSQL-backed tests for /start-fulfillment bot command (Tier 2 — direct write).

Covers:
- Successful RESERVED → FULFILLING
- Correct actor/representative is used
- Cross-representative isolation / IDOR protection
- Non-RESERVED order is rejected (DRAFT, PENDING_APPROVAL, APPROVED,
  BACKORDERED, FULFILLING, SHIPPED, CANCELLED)
- Unknown order number
- Missing/invalid command argument
- Missing BOT_WRITE permission
- History/audit behavior not duplicated
- Repeated execution / already-transitioned order
...
requires_database = pytest.mark.skipif(
    ...
    reason="DATABASE_URL not set; skipping /start-fulfillment tests",
)
...
def _create_representative(session: Session, su) -> Representative:
    ...
    rep = Representative(
        code=f"REP-SF-{suffix.upper()}",
        person_name=f"StartFulfill Rep {suffix}",
        status="ACTIVE",
        created_by=su.id,
        updated_by=su.id,
    )
...
def _create_app_user(session: Session, su, rep: Representative) -> AppUser:
    ...
    return auth_service.create_user(
        ...
        email=f"sf_{suffix}@test.invalid",
...
def _grant_permission(session, app_user, su, perm_code):
    suffix = uuid.uuid4().hex[:8]
    role_code = f"SF_{perm_code}_{suffix}"
    rbac_service.create_role(
        session, code=role_code, name=f"SF {perm_code} {suffix}",
        created_by=su.id,
    )
    try:
...
def _create_product(session: Session, su, prefix="SKU-SF"):
    suffix = uuid.uuid4().hex[:8]
    product = Product(
        sku=f"{prefix}-{suffix}",
        name=f"StartFulfill Product {suffix}",
...
def _create_draft_order(session, su, rep, product, *, qty=10):
    ...
    customer = Customer(
        code=f"C-SF-{suffix}",
        name=f"StartFulfill Customer {suffix}",
        type="CORPORATE",
        currency_id=currency.id,
        status="ACTIVE",
        created_by=su.id,
...
def _create_reserved_order(session, su, rep, product, *, qty=10):
    """Create an order in RESERVED state."""
...
# =======================================================================
# 1. Successful RESERVED → FULFILLING
# =======================================================================


@requires_database
class TestStartFulfillmentSuccess:
    def test_reserved_order_starts_fulfillment(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-ok-{uuid.uuid4().hex[:6]}"
            ...
            order = _create_reserved_order(session, su, rep, product)
    ...
    def test_resulting_state_is_fulfilling(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-state-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            product = _create_product(session, su)
            order = _create_reserved_order(session, su, rep, product)
            ...
            response = process_message(session, message=msg)
            assert "FULFILLING" in response.text
        finally:
            session.close()


# =======================================================================
# 2. Correct actor recorded
# =======================================================================


@requires_database
class TestStartFulfillmentActor:
    def test_actor_recorded_correctly(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-actor-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            product = _create_product(session, su)
            order = _create_reserved_order(session, su, rep, product)
            ...
            assert last.to_state == "FULFILLING"
...
# =======================================================================
# 3. Cross-representative isolation / IDOR
# =======================================================================


@requires_database
class TestStartFulfillmentIDOR:
    def test_rep_a_cannot_fulfill_rep_b_order(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)

            puid_a = f"sf-idora-{uuid.uuid4().hex[:6]}"
            rep_a, user_a, _ = _make_bound_session(
                session, su, platform_user_id=puid_a
            )
            _grant_bot_write(session, user_a, su)

            puid_b = f"sf-idorb-{uuid.uuid4().hex[:6]}"
            ...
            order_b = _create_reserved_order(session, su, rep_b, product)
            ...
            msg = BotMessage(
                ...
                text=f"/start-fulfillment {order_b.order_number}",
            )
            response = process_message(session, message=msg)
...
# =======================================================================
# 4. Non-RESERVED states rejected
# =======================================================================


@requires_database
class TestStartFulfillmentNonReserved:
    def _test_rejects_state(self, target_state):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-rej-{target_state[:4].lower()}-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            ...
            response = process_message(session, message=msg)
            assert "cannot start fulfillment" in response.text.lower()
            session.refresh(order)
            assert order.state == target_state
        finally:
            session.close()
    ...
    def test_rejected_when_backordered(self):
        self._test_rejects_state("BACKORDERED")
...
@requires_database
class TestStartFulfillmentUnknown:
    def test_unknown_order_not_found(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-ne-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)

            msg = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text="/start-fulfillment ORD-NONEXISTENT-123",
            )
            response = process_message(session, message=msg)
            assert "not found" in response.text.lower()
        finally:
...
@requires_database
class TestStartFulfillmentMissingArgs:
    def test_usage_hint_when_no_args(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-noarg-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
...
# =======================================================================
# 7. Missing BOT_WRITE permission
# =======================================================================


@requires_database
class TestStartFulfillmentPermission:
    def test_rejected_without_bot_write(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-noperm-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            # Only BOT_QUERY, no BOT_WRITE.

            msg = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text="/start-fulfillment ORD-TEST",
            )
...
# =======================================================================
# 8. Audit/history not duplicated
# =======================================================================


@requires_database
class TestStartFulfillmentNoDuplicateAudit:
    def test_exactly_one_history_entry_added(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-audit-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            product = _create_product(session, su)
            order = _create_reserved_order(session, su, rep, product)
            ...
            # Audit log: exactly one new UPDATE for the transition.
            ...
            assert len(audit) >= 1
            last_audit = audit[-1]
            assert "FULFILLING" in str(last_audit.after_json)
        finally:
            session.close()


# =======================================================================
# 9. Repeated execution / already-transitioned
# =======================================================================


@requires_database
class TestStartFulfillmentRepeated:
    def test_already_fulfilling_rejected(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-rep-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            product = _create_product(session, su)
            order = _create_reserved_order(session, su, rep, product)

            # First start-fulfillment — should succeed.
            msg1 = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text=f"/start-fulfillment {order.order_number}",
            )
            response1 = process_message(session, message=msg1)
            assert "fulfillment started" in response1.text.lower()

            # Second start-fulfillment — should fail (already FULFILLING).
            msg2 = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text=f"/start-fulfillment {order.order_number}",
            )
            response2 = process_message(session, message=msg2)
            assert "cannot start fulfillment" in response2.text.lower()
        finally:
            session.close()
...
@requires_database
class TestStartFulfillmentNoUUID:
    def test_no_uuids_in_response(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-uuid-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            _grant_bot_write(session, user, su)
            product = _create_product(session, su)
            order = _create_reserved_order(session, su, rep, product)
...
@requires_database
class TestStartFulfillmentRegression:
    def test_help_includes_start_fulfillment(self):
        session = get_session_factory()()
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-reghelp-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            msg = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text="/help",
            ...
            response = process_message(session, message=msg)
            assert "/start-fulfillment" in response.text
        finally:
            session.close()
    ...
    def test_orders_still_works(self):
        ...
        try:
            bootstrap_service.ensure_rbac_bootstrap(session)
            su = bootstrap_service.ensure_system_user(session)
            puid = f"sf-regord-{uuid.uuid4().hex[:6]}"
            rep, user, _ = _make_bound_session(session, su, platform_user_id=puid)
            msg = BotMessage(
                platform_user_id=puid, platform_code="TELEGRAM",
                text="/orders",
