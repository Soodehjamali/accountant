...
class AppUser(Base, UniversalAuditColumns):
    ...
    # use_alter=True: representative (via UniversalAuditColumns'
    # created_by/updated_by) FKs back to app_user.id, so this is one leg of a
    # circular FK pair. use_alter defers this constraint to a post-create
    # ALTER TABLE ADD CONSTRAINT (both in Alembic's autogenerated migration
    # and in any plain create_all()), breaking the CREATE TABLE cycle.
