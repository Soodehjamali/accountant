...
class Representative(Base, UniversalAuditColumns):
    ...
    # use_alter=True: commission_config.representative_id FKs back to
    # representative.id, and commission_config (via UniversalAuditColumns)
    # also FKs to app_user.id, which in turn FKs to representative.id --
    # this column closes a circular-FK cycle (representative ->
    # commission_config -> app_user -> representative). use_alter defers it
    # to a post-create ALTER TABLE ADD CONSTRAINT, the same fix applied to
    # app_user.representative_id for the analogous app_user<->representative
    # cycle.
