...
* **No real ``ForeignKey()`` constraints yet.** ``created_by`` / ``updated_by``
  are plain ``UUID`` columns (``app_user`` model does not exist yet); the FK
  will be added by a later migration once the ``app_user`` model lands. This is
  an explicit, time-bounded deviation from full normalization — recorded here
...
class UniversalAuditColumns:
    ...
    Notes:
        ...
        * Neither ``created_by`` nor ``updated_by`` is a ``ForeignKey()`` —
          ``app_user`` does not exist yet. A migration adds the FKs later.
    ...
    #: Last mutator (NULL = system-generated UPDATE). No FK until app_user lands.
...
class AppendOnlyAuditColumns:
    ...
    Notes:
        ...
        * ``created_by`` is a plain ``UUID``, **not** a ``ForeignKey()`` —
          ``app_user`` does not exist yet; the FK is added by a later migration.
    """
    ...
    #: Posting actor (NULL = system-generated). No FK until app_user lands.
