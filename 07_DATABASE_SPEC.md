...
7. **Business Constraints:** May aggregate lines from multiple orders, resolved via `invoice_order` (J1); once `state IN ('ISSUED','PARTIALLY_PAID','PAID','CLOSED_CORRECTED')` (i.e. any state other than `DRAFT` or `VOID`) the header and its lines are immutable — enforced via `BEFORE UPDATE` trigger (corrected by `09_Decisions.md` ADR-006 from an earlier, inconsistent `PAID`/`CLOSED_CORRECTED`-only condition, to match this same section's own Soft Delete Strategy boundary below and `invoice_line`'s (T18) stated `ISSUED` boundary); corrections are issued only via `credit_note` (T20), never a direct edit; `amount_paid`/`balance_due` remain writable post-`ISSUED` as a column-level exception to this trigger, but **only** by the reconciliation job that sums `payment_allocation`, never by general application code — enforced by column-level GRANT restricting `UPDATE (amount_paid, balance_due)` to the reconciliation service role.
